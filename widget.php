...
if ( ! class_exists( 'ITTabberWidget' ) ) {
	class ITTabberWidget extends WP_Widget {
		...
		function __construct() {
			...
			parent::__construct( $this->_var, $this->_widget_name, $widget_ops, $control_ops );
...
//			wp_enqueue_style( 'it-classes-style', "{$this->_plugin_url}/lib/classes/css/classes.css" );
		...
		function _set_vars() {
			...
			$this->_plugin_url = preg_replace( '|^https?://|', '//', get_option( 'siteurl' ) . '/' . $this->_plugin_relative_path );
		}
