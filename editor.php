...
Change Notes:
	1.0.1 - 2009-11-20
		Changed the sub-widgets from h4 to h5 titles
	1.0.2 - 2010-01-13
		Added _default_menu_function to move it to the Tools menu
	1.0.3 - 2013-12-11 - Chris Jean
		Removed sources of warnings.
*/
...
if ( ! class_exists( 'ITTabber' ) ) {
	class ITTabber extends ITCoreClass {
		...
		function init() {
			...
			if ( ! empty( $this->_options['tabbers'] ) ) {
				foreach ( (array) $this->_options['tabbers'] as $id => $tabber ) {
					for ( $counter = 1; $counter <= $tabber['num']; $counter++ ) {
						@register_sidebar( array(
							'name'          => "Tabber - {$tabber['name']} - {$tabber["title_$counter"]}",
							'before_widget' => '<div class="tabber-widget %2$s" id="%1$s">',
							'after_widget'  => '</div>',
							'before_title'  => '<h5 class="tabber-widget-title">',
							'after_title'   => '</h5>'
						) );
					}
				}
...
?>
	<?php if ( empty( $this->_errors ) && ! isset( $_REQUEST['id'] ) ) : ?>
		<div class="wrap">
			...
			<?php if ( ! empty( $this->_options['tabbers'] ) && ( count( $this->_options['tabbers'] ) > 0 ) ) : ?>
				<table class="widefat">
					<thead>
						<tr class="thead">
							<th>Name</th>
